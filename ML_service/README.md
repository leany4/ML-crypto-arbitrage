...
| `hybrid_q35_transformer_rl` | Legacy q35 | Transformer выбирает вход, RL - выход |
| `rl_paper` | Legacy q35 gate | Действия recurrent RL |

По умолчанию активны три положительные offline-комбинации:
`transformer_full`, `hybrid_q35_transformer` и
`hybrid_q35_transformer_rl`. Стратегии можно включать и приостанавливать через
API или dashboard.
...
Проверка Transformer latency после CUDA warm-up:

```bash
docker compose -f compose.yaml -f compose.cuda.yaml exec ml-service \
  python /app/scripts/benchmark_transformer_latency.py \
  --device cuda \
  --warmup 10 \
  --iterations 100 \
  --max-p95-ms 100
...
Для cadence 100 мс ориентируйтесь на `full_history.p95_ms`, а не на единичный
холодный запуск или только `tensor_ready`.
...
Репозиторий содержит причинный трёхдневный test replay:

```text
market_replay_data/raw_last_3d/
├── l2_raw.parquet
├── ohlcv_raw.parquet
└── manifest.json
```

Parquet-файлы хранятся через Git LFS. После обычного `git clone` убедитесь, что
установлен Git LFS и выполните `git lfs pull`.
